...
class Encoder(nn.Module):
    ...
    def forward(self, X, X_lengths):
        # Pack the padded sequences to handle variable sequence lengths
        packed_input = pack_padded_sequence(X, X_lengths.cpu(), batch_first=True, enforce_sorted=False)
        
        # Initialize hidden and cell states (for an LSTM, we need both)
        ...
        packed_output, (hidden, cell) = self.lstm(packed_input, (hidden, cell))
        
        # Unpack the output sequence
        output, _ = nn.utils.rnn.pad_packed_sequence(packed_output, batch_first=True)
        
        return output, (hidden, cell)  # Return encoder outputs and hidden states
...
class Decoder(nn.Module):
    def __init__(self, hidden_size, output_size, num_layers=1):
        ...
        # LSTM for decoding
        self.lstm = nn.LSTM(input_size=hidden_size, hidden_size=hidden_size, num_layers=num_layers, dropout = 0.2, batch_first=True)
        
        # Fully connected layer to project hidden state to output size
        self.fc = nn.Linear(hidden_size, output_size)
    def forward(self, context, output_length):
        """
        context: Tensor of shape [batch_size, seq_len, hidden_size] from the encoder's output for entire sequence
                 
        output_length: int
                       The number of time steps to decode.
        Returns:
            Y_pred: Tensor of shape [batch_size, output_length, output_size]
                    The sequence of predictions for the given output length.
        """
        batch_size = context.size(0)
        # Use the last hidden state from the encoder as the initial input
        decoder_input = context[:, -1, :].unsqueeze(1)  # [batch_size, 1, hidden_size]

#         print(decoder_input[0])
        # Initialize LSTM hidden and cell states
        hidden = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(context.device)
        cell = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(context.device)

        outputs = []
#         print("DECODER INPUT SHAPE BEFORE FC: " + str(decoder_input.shape))
#         print(decoder_input.shape)
        for t in range(output_length):
            # We pass the decoder input and previous hidden states to the LSTM
            output_lstm, (hidden, cell) = self.lstm(decoder_input, (hidden, cell))  # output: (batch_size, 1, hidden_size)
            
#             print(" OUTPUT SHAPE BEFORE FC: " + str(output_lstm.shape))
#             print(" OUTPUT SHAPE AFTER SQUEEZE FC: " + str(output_lstm.squeeze(1).shape))
            # The output from the LSTM is passed through a fully connected layer to map to the desired output shape
            output = self.fc(output_lstm.squeeze(1))  # Remove the sequence dimension (output: batch_size, output_size)
#             print(" OUTPUT SHAPE AFTER FC: " + str(output.shape))
            outputs.append(output)
            
#             print(output.shape)
            # Decoder input for the next timestep is the current output
            decoder_input = output_lstm
#             print("DECODER INPUT SHAPE AFTER FC: " + str(decoder_input.shape))
        return torch.stack(outputs, dim=1)  # Stack all the outputs along the sequence length dimension
...
class Seq2SeqModel(nn.Module):
    ...
    def forward(self, X, X_lengths, Y_length):
        # Pass input through encoder
        encoder_outputs, (hidden, cell) = self.encoder(X, X_lengths)
#         print(encoder_outputs.shape)
        # Decoder takes encoder hidden state ouput and generates Y_length steps of prediction
        Y_pred = self.decoder(encoder_outputs, Y_length)
        
        return Y_pred


def trainED(hidden_size, num_layers, num_epochs, train_data_loader):
    # Hyperparameters
    input_size = 13   # Number of features in each frame
    output_size = 13  # Number of features in each frame of the output sequence
    batch_size = 2
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = Seq2SeqModel(input_size, hidden_size, output_size, num_layers).to(device)
...
test_dataset = {}
def evaluateED(model, norm_test):
    model.eval()
